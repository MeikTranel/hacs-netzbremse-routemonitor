...
This document describes all configuration options and settings available in the Netzbremse Routemonitor custom integration.
...
### `netzbremse_routemonitor.example_service`
...
```yaml
service: netzbremse_routemonitor.example_service
target:
  ...
  - alias: "Call service at sunset"
    trigger:
      - trigger: sun
        event: sunset
    action:
      - action: netzbremse_routemonitor.example_service
        target:
          entity_id: switch.device_name_switch
        data:
          parameter: "mode"
...
```yaml
blueprint:
  name: Netzbremse Routemonitor Alert
...
- [GitHub Issues](https://github.com/MeikTranel/hacs-netzbremse-routemonitor/issues) - Report problems
