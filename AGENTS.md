...
- **Repository:** MeikTranel/hacs-netzbremse-routemonitor

**Key directories:**

- `custom_components/netzbremse_routemonitor/` - Main integration code
...
- Integration logs: `custom_components.netzbremse_routemonitor: debug` in `config/configuration.yaml`
...
- Use the domain `netzbremse_routemonitor` for all DOMAIN references
- Prefix all integration-specific classes with `NetzbremseRoutemonitor`
- Use "Netzbremse Routemonitor" as the display title
...
- Inherit from platform base + `NetzbremseRoutemonitorEntity`
...
- `tests/` mirrors `custom_components/netzbremse_routemonitor/` structure
