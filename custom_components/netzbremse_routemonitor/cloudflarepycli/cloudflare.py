...
class TestTimers(NamedTuple):
    ...
    def jitter_from(latencies: list[float]) -> float | None:
        ...
        if len(latencies) < 2:
            return None
        return statistics.mean([abs(latencies[i] - latencies[i - 1]) for i in range(1, len(latencies))])
...
def _with_units(base_label: str, value: float, *, megabits: bool) -> tuple[str, TestResult]:
...
class CloudflareSpeedtest:
    ...
    def __init__(
        ...
        - `timeout`: The timeout in seconds for all requests. Defaults to 120.
    ...
    async def run_all(self, *, megabits: bool = False) -> SuiteResults:
        ...
        for k, v in data.items():
            ...
            label, value = _with_units(f"90th_percentile_{k}", result or 0, megabits=megabits)
            self._sprint(
                label,
    ...
    def results_to_dict(
        ...
        return {k: {sk: sv._asdict()} for k, v in results.items() for sk, sv in v.items()}
